...
export default function Estimate(props) {
  ...
  const [dialogOpen, setDialogOpen] = useState(false);const [name, setName] = useState('');
  ...
  const getCustomFeatures = () => {
    if (questions.length > 2) {
      const newCustomFeature = questions.filter(question => question.title === 'What type of custom features do you expect to need?')
  ...
  return (
    <Grid container direction='row'>
      <Grid item container direction='column' lg>
        ...
        <Grid item>
          <Lottie options={defaultOptions} height='100' width='100' style={{marginRight: '10em', maxWidth: '50em', marginTop: '7.5em'}}/>
        </Grid>
      </Grid>
          ...
          <React.Fragment key={index}>
            ...
            <Grid item container direction='row'>
              {question.options.map((option, index) => (
                <Grid item container 
                  direction='column' md 
                  key={index} component={Button} style={{borderRadius: 0, display: "grid", textTransform: 'none', backgroundColor: option.selected ? theme.palette.common.orange : null}} onClick={() => handleSelect(option.id)}>
        ...
        <Grid item container justify='space-between' style={{width: '18em', marginTop: '3em'}}>
          ...
          <Grid item> 
            <IconButton disabled={navigationNextDisabled()} onClick={nextQuestion}>
              <img src={navigationNextDisabled() ? forwardArrowDisabled : forwardArrow} alt='previous question' />
              </IconButton>
          </Grid>
        </Grid>
        <Grid item>
          <Button variant='contained' className={classes.estimateButton} onClick={() => {setDialogOpen(true); getTotal(); getPlatforms(); getFeatures(); getCustomFeatures(); getCategory();}}>
            Get Estimate
          </Button>
        </Grid>
      </Grid>
        ...
        <DialogContent>
          <Grid container>
            <Grid item container direction='column' md={7}>
              ...
              <Grid item>
                <Typography variant='body1' paragraph>We can create this digital estimate solution for an estimated <span className={classes.specialText}>${total.toFixed(2)}</span></Typography>
            ...
            <Grid item container direciton='column' md={5}>
              <Grid item>
